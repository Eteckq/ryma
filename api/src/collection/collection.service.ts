...
export class CollectionService {
  ...
  async generateRandomItemForCollection(user: User, id: string) {
    ...
    if (user.remainingRolls == 0)
      throw new ForbiddenException('No remaining rolls');
    ...
    if (collection.items.length == 0)
      throw new ForbiddenException('Collection is empty');

    const randomRarity = getRandomRarity(
      eliminerDoublonsParId(collection.items.map((i) => i.rarity)),
    );
    if (!randomRarity)
      throw new NotFoundException('Unable to get random rarity (wtf)');

    const reducedCollection = collection.items.filter(
      (i) => i.rarity.id == randomRarity.id,
    );
    const item =
      reducedCollection[Math.floor(Math.random() * reducedCollection.length)];
