<template>
    <div class="flex items-center justify-center w-full">
        <label for="dropzone-file" @dragover="dragover" @dragleave="dragleave" @drop="drop"
            ...
            <input @change="handleFileUpload($event)" id="dropzone-file" type="file" class="hidden" />
        </label>
    </div>
</template>
...
export default {
    ...
    methods: {
        async submitFile() {
            ...
            try {
                const res = await this.$axios.post('/api/file/upload',
                    formData,
                    {
                        headers: {
